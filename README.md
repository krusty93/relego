...
`docker compose --profile app up -d` starts **relego-server**, which serves both the API and the web UI on <http://localhost:8080>. It is the easiest way to run Relego day to day: everything the TUI does, in the browser, on desktop or phone.
...
This starts the API and the [web UI](#web-ui) together on <http://localhost:8080>. Steps 3 to 6 below use the CLI; you can do all of them in the browser instead.
...
| Web UI shows "Disconnected" | The browser cannot reach the server. Check `docker compose ps` and open <http://localhost:8080>. |
